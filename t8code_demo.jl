...
# ╔═╡ 0affbd58-3a70-4ec3-b150-9f517a05dedd
using Plots

# ╔═╡ 9a50497b-7c27-434e-8d53-e5119c0ab4bb
using LaTeXStrings

# ╔═╡ c302a0a4-f920-45c2-b2c2-81b59a88c704
using StaticArrays
...
# ╔═╡ d797c07f-c192-4000-a17d-6829f6dc730b
md"""
If you want to know more about `t8code/p4est`, you can look at the following resources:

- [`p4est` website](https://www.p4est.org)
- [`p4est` repository](https://github.com/cburstedde/p4est)
- [P4est.jl repository](https://github.com/trixi-framework/P4est.jl)
- [`t8code` repository](https://github.com/DLR-AMR/t8code)
- [T8code.jl repository](https://github.com/DLR-AMR/T8code.jl)

Furthermore, the following repositories use the Julia bindings:
- [Trixi.jl](https://github.com/trixi-framework/Trixi.jl) implements adaptive high-order methods for conservation laws
- [MAGEMinApp.jl](https://github.com/ComputationalThermodynamics/MAGEMinApp.jl) implements a GUI for MAGEMin and uses AMR to locate phase boundaries
"""

# ╔═╡ 88eaa2e5-841c-49e4-b9d2-79b85bfdc23b
md"""
!!! warning "Simplification"
	This simple introduction performs several simplifications and requires
    - 2D Cartesian quad meshes
    - serial execution (no MPI)
    - a single data point per mesh cell
"""
...
const comm = let
	...
	# Only initialize t8code if it hasn't been initialized already
...
# ╔═╡ 2874c5a5-053e-44d3-8d9c-e2918b4c82e3
md"""
## Construct a uniformly refined mesh
"""

# ╔═╡ d51d9c33-c181-40e0-a745-b980ad445407
function create_forest(; x, y, level)
    # Vertices of the quad in Z order
    vertices = [
        x[1], y[1], 0,
	    x[2], y[1], 0,
	    x[1], y[2], 0,
    ...
    # Define of the class of the tree
	...
	# Create a forest with uniform refinement
	forest = t8_forest_new_uniform(cmesh, t8_scheme_new_default_cxx(), level, 0, comm)

	return forest
end

# ╔═╡ 63671411-a0be-4661-9b94-e267022b784f
# Coordinates of the enclosing rectangle
forest = create_forest(x = (0.0, 15.0), y = (0.0, 10.0), level = 2)

# ╔═╡ d0ca6936-4c60-4f6b-a893-9bc3195b1ccc
t8_forest_get_local_num_elements(forest)

# ╔═╡ de25f054-f07f-42a4-8a9f-855d49d1e039
t8_forest_get_global_num_elements(forest)

# ╔═╡ 6df9f453-74f9-4171-a02e-f81165c89105
md"""
## Visualize a uniform mesh
"""

# ╔═╡ 60e483be-1318-450e-a8cd-35fd2d7c4b5b
md"""
## Discretize data on a uniform mesh and visualize them together with the mesh
"""

# ╔═╡ cfc4a6d9-f93e-4147-b1ea-6cd1e4403c68
md"""
## Refine a mesh locally
"""

# ╔═╡ 135294ed-bc29-494f-bd26-48fdf83da11d
function simple_adapt_callback(forest, forest_from, which_tree, lelement_id,
                        	   ts, is_family, num_elements, elements_ptr) :: Cint
	# Our adaptation criterion is given by the vector `refine_elements`,
	# passed as user data
...
# ╔═╡ 891aca44-d9fc-44e0-99e6-210d2b3602ca
md"""
## Refine a mesh based on geometric information

👉 Create a mesh and refine all cells based on geometric information, e.g.,
refine all cells within a circle of a given radius.
"""

# ╔═╡ 3a0eab5a-5997-459c-9ca8-8767b0b9c0d0
md"""
## Refine a mesh based on discretized data

👉 Create a mesh, discretize some data on it, refine the mesh based on the values of the data, and re-evaluate the data on refined cells.
"""
...
# ╔═╡ 4e665fd6-862e-4a44-a54e-b9ea598a07af
space

# ╔═╡ 7e33136c-ba54-4289-a274-9424475ab02f
space

# ╔═╡ 1fa71bf1-8b8d-4940-8467-65c69041632d
space

# ╔═╡ 5cadaccc-f1f2-478f-bba3-dbb644669c74
space

# ╔═╡ b3b027b3-c91f-4ae4-99de-7b2e46aa22a7
space
...
"""
	get_maximum_level(forest)

Return the maximal refinement level of all (local) elements.
"""
...
"""
	get_element_data(forest)

Return data with the following fields
- `x_cell`: Coordinates of the cell centers
- `x_edge`: Coordinates of the edges of each cell
- `tree_id`: local tree ID of each cell
- `element_id`: local element ID of each cell
- `element_level`: refinement level of each cell
- `unique_element_id`: globally unique element ID of each cell

!!! warning "Simplification"
	The current version requires 2D Cartesian quad meshes
"""
function get_element_data(forest)
	...
	num_local_trees = t8_forest_get_num_local_trees(forest)
	for itree in 0:(num_local_trees - 1)
		@T8_ASSERT t8_forest_get_eclass(forest, itree) == T8_ECLASS_QUAD
		@T8_ASSERT t8_forest_get_tree_class(forest, itree) == T8_ECLASS_QUAD
	end
	ndims = 2
	num_faces = 2^ndims
    ...
    x_cell = Vector{SVector{ndims, Float64}}(undef, num_local_elements)
    ...
    element_coords_ll = Array{Float64}(undef, ndims)
    element_coords_lr = Array{Float64}(undef, ndims)
    element_coords_ul = Array{Float64}(undef, ndims)
    element_coords_ur = Array{Float64}(undef, ndims)
    ...
    return (; x_cell, x_edge, tree_id, element_id, element_level, unique_element_id)
end

# ╔═╡ 09b3194c-685e-4d9e-84df-fd06eaad3927
let data = get_element_data(forest)
	# Unique coordinates of the cell centers
	x = getindex.(data.x_cell, 1) |> unique |> sort
	y = getindex.(data.x_cell, 2) |> unique |> sort

	# Plots.jl requires data to be sorted lexocigraphically
...
# ╔═╡ 420ac38d-86dd-4bb2-a1e0-b0bf94bf8acb
let data = get_element_data(forest)
	# Unique coordinates of the cell centers
	x = getindex.(data.x_cell, 1) |> unique |> sort
	y = getindex.(data.x_cell, 2) |> unique |> sort
	...
	# Plot edges as lines
	x_edge = Vector{Float64}(undef, 6 * length(data.x_edge))
	y_edge = Vector{Float64}(undef, 6 * length(data.x_edge))
	i = 1
	for cell in eachindex(data.x_edge)
		# Plots.jl requires clockwise or counterclockwise
		# order but p4est gives Z order
		x_edge[i + 0], y_edge[i + 0] = data.x_edge[cell][1]
		x_edge[i + 1], y_edge[i + 1] = data.x_edge[cell][2]
		x_edge[i + 2], y_edge[i + 2] = data.x_edge[cell][4]
		x_edge[i + 3], y_edge[i + 3] = data.x_edge[cell][3]
		x_edge[i + 4], y_edge[i + 4] = data.x_edge[cell][1]
		# Separate edges with NaN
		x_edge[i + 5], y_edge[i + 5] = NaN, NaN
		i += 6
	end
...
"""
	plot_forest_data(forest, values::Vector{Cdouble};
                     data = get_element_data(forest),
                     grid = true, kwargs...)

Plot the `values` associated to each cell of the mesh given by the `forest`
as a `heatmap`. If `grid == true`, the grid lines are also shown. Other
keyword arguments `kwargs...` are passed forward to `heatmap` from Plots.jl.

This is a simple plotting routine that should not be used for performance-critical
applications. In particular with MPI parallelization, you should use other means,
e.g., VTK output and paraview.
"""
function plot_forest_data(forest, values::Vector{Cdouble};
						  data = get_element_data(forest),
						  grid = true, kwargs...)
	@T8_ASSERT t8_forest_is_committed(forest) == 1
	...
	# Required to keep the old forest alive when refining
	...
	while true
		...
		if t8_forest_get_local_num_elements(forest_adapt_1) ==
	...
	if grid
		...
		plot!(fig, x_edge, y_edge; color = :gray, label = "")
	end

	return fig
end

# ╔═╡ 4a5d092a-cfb2-4343-862c-6224bd9c914f
let forest = create_forest(x = (0.0, 15.0), y = (0.0, 10.0), level = 2)
...
md"""
### Cells below were used for development
"""

# ╔═╡ 11b58f1a-0ff2-40cb-a9fb-b8d166499aa3
# ╠═╡ disabled = true
#=╠═╡
let forest = create_forest(x = (0.0, 15.0), y = (0.0, 10.0), level = 1)
	    num_local_trees = t8_forest_get_num_local_trees(forest)
	...
	data = get_element_data(forest)
	values = Cdouble.(data.unique_element_id)
	...
	while true
		...
		# t8_forest_unref(forest_adapt_1)
		# t8_forest_unref(forest_adapt_2) ?
		# @info "destroyed" t8_forest_is_committed(forest) t8_forest_is_committed(forest_adapt_1) t8_forest_is_committed(forest_adapt_2)
...
manifest_format = "2.0"
project_hash = "508608332c6c109691ffac7e98f90021290f388f"
...
[[deps.StatsBase]]
deps = ["DataAPI", "DataStructures", "LinearAlgebra", "LogExpFunctions", "Missing", "Printf", "Random", "SortingAlgorithms", "SparseArrays", "Statistics", "StatsAPI"]
